...
- Primo uses custom1.css from this directory in it's view package, but custom1.css is not part of this repository?
- custom1.css will be automatically generated by concatenating the .css files in here.
- Generation will happen:
  - When the package is created `docker-compose run server gulp create-package`
  - Manually with `docker-compose run server gulp custom-css`
  - Automatically on .css file change while the container is running
